...
from instruct_llama.utils.file_helper import read_zipped_jsonl_file, find_certain_files_under_dir
...
def _save_dataset_to_disk(metadata, output_dir, data_type, dataset_prefix, num_tokens, temp_files, logger, shuffle=True):
    ...
    _merge_and_write_to_disk()
    

def _merge_and_write_to_disk(temp_files, data_type, max_items, output_file, delete_temp_files=True):
    """
    Combining different temp files together and save content into a single numpy memmap array.
    """
...
def process_single_file(
    input_file: str, tokenizer: Tokenizer, validation_ratio: float, output_dir: str, max_chuk_size: int = 2
):
    ...
    for row in read_zipped_jsonl_file(input_file):
        ...
        # Write current chunk to disk to free RAM
        # one integer takes 28 bytes, so 1GB RAM = 1e9 bytes
        if len(tokens[ds_prefix] * 28 >= max_chuk_size * 1e9):
...
def process_redparjama_dataset(
    ...
    """
    Process Read Pajama dataset and save the tokenized text to .pkl format.
    ...
    logger.info(f"Processing {num_files} file using {num_workers} workers ...")
    ...
    data_type = np.uint16 if tokenizer.vocab_size < 2**16 else np.unint32
